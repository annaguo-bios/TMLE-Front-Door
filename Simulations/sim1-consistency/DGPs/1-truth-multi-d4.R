...
compute_psi_and_var <- function(a, parA, parU, parM, parY,n){
  ...
  p.M.AX <- sapply(1:n, function(i){density.m.ax(parM,c(M.1[i],M.2[i],M.3[i],M.4[i]),A[i],X[i])})  #p(M|A,X)

  # outcome regression E[Y|M,A,X] 
  mu <- E.y.max(parY,parU,cbind(M.1,M.2,M.3,M.4),A,X)
  mu.a1 <- E.y.max(parY,parU,cbind(M.1,M.2,M.3,M.4),1,X)
  mu.a0 <- E.y.max(parY,parU,cbind(M.1,M.2,M.3,M.4),0,X)
