...
compute_psi_and_var <- function(a, parA, parU, parM, parY,n){
  ...
  p.M.altX <- sapply(1:n, function(i){density.m.ax(parM,c(M.1[i],M.2[i]),alt,X[i])})  #p(M|A,X)

  # outcome regression E[Y|M,A,X]
  mu.alt <- E.y.max(parY,parU,cbind(M.1,M.2),alt,X)

  ## int E(Y|M,A,X)p(M|A=a,X)p(A|X)dMdA

  # E(Y|M,A,X)p(M|A=a,X)
  mu.M <- function(m,x,A){E.y.max.single(parY,parU,m,alt,x)*density.m.ax(parM,m,a,x)}

  # int E(Y|M,A=a',X)p(M|A=a,X)
  kappa <- parY[1]*{parU[1] + parU[2]*alt + parU[3]*X + parU[4]*alt*X} +
