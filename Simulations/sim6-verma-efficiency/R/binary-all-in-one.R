...
generate_data <- function(n,parA = c(0.3,0.2), parU=c(1,1,-0.2,0), parM = c(-1,1,1,0), parY = c(1, 1, 0, 0), sd.U=1, sd.Y=1){ # change the parM to c(-1,1,1,0)
  ...
  Y <- parY[1]*U + parY[2]*M + parY[3]*Z + rnorm(n, 0, sd.Y) # p(Y|U,M)
...
#   
#   f.AZ <- function(A,Z,a){
#     (1+A+1+0.8*Z)*plogis(-1+a+Z)*{A*(0.3+0.2*Z) + (1-A)*(0.7-0.2*Z)}*(0.8-0.6*Z)+
#       (1+A+0+0.8*Z)*(1-plogis(-1+a+Z))*{A*(0.3+0.2*Z) + (1-A)*(0.7-0.2*Z)}*(0.8-0.6*Z)
#   }
#   
#   res <- f.AZ(0,0,a) + f.AZ(1,0,a) + f.AZ(0,1,a) + f.AZ(1,1,a)
#   
#   return(res)
#   
# }
...
E.y.maz <- function(m,a,z){
  parY = c(1, 1, 1, 0)
  parU=c(1,1,-0.2,0)
  parY[1]*{parU[1] + parU[2]*a + parU[3]*z + parU[4]*a*z} + parY[2]*m + parY[3]*z
}
...
f.onestep.binaryZM <- function(a,z=NULL){ # 1. if z=null, then return estimand from the regular ID functional where z is not fixed, 2. if z is not null, then return the estimand from the ID functional where z is fixed
  ...
  if (!is.null(z)){p.M.az <- density.m.az(M,a,z)} # p(M|A=a,Z=z)
