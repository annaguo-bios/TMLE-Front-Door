...
load("binaryM/Onestep-est1/result.Rdata")
...
# Binary M, Estimator1-----------------
load("binaryM/TMLE-est1/result.Rdata")
...
load("continuousM/Onestep-est2-dnorm/result.Rdata")
...
load("continuousM/TMLE-est2-dnorm/result.Rdata")
...
load("continuousM/Onestep-est2a/result.Rdata")
...
load("continuousM/TMLE-est2a/result.Rdata")
...
load("continuousM/Onestep-est2b/result.Rdata")
...
load("continuousM/TMLE-est2b/result.Rdata")
...
load("multiM-d2/Onestep-est2a/result.Rdata")
...
load("multiM-d2/TMLE-est2a/result.Rdata")
...
load("multiM-d2/Onestep-est2b/result.Rdata")
...
load("multiM-d2/TMLE-est2b/result.Rdata")
...
# Multivariate M, Onestep-bayes
load("multiM-d2/Onestep-est2-dnorm/result.Rdata")
...
# Multivariate M, Estimator3
load("multiM-d2/TMLE-est2-dnorm/result.Rdata")
...
#   insert_row("","","","","","","","","","","","","","",after=9) %>%
#   insert_row("","","","","","","","","","","","","","",after=15) %>%
#   insert_column(c("","","Submodels","n=500","Bias","SD","MSE","CI coverage","CI width","n=1000","Bias","SD","MSE","CI coverage","CI width","n=2000","Bias","SD","MSE","CI coverage","CI width"), after = 0) %>%
...
#   set_right_border(4:nrow(.), 3, brdr(0.4,"double")) %>%
#   set_right_border(4:nrow(.), 5, brdr(0.4,"dotted")) %>%
#   set_right_border(4:nrow(.), 7, brdr(0.4,"dotted")) %>%
#   set_right_border(4:nrow(.), 9, brdr(0.4,"double")) %>%
#   set_right_border(4:nrow(.), 11, brdr(0.4,"dotted")) %>%
...
#   set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>% set_caption("Comparison between TMLEs for ATE using linear and logit submodels under binary, continuous, and multivariate mediators.") %>%
...
table1 <- as_hux(dat) %>% 
  ...
  set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>%
  set_top_border(row=1,col=everywhere,brdr(1, "solid")) %>% set_bottom_border(row = nrow(.),col = everywhere,brdr(1, "solid")) %>%
  set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>% set_caption("Comparison between TMLEs for ATE using linear and logit submodels across mediator types.") %>%
  set_all_padding(1) %>% set_font_size(6) 
