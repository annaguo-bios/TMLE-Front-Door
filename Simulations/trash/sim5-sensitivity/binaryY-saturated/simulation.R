...
sample_sizes <- c(1000, 1500, 2000, 5000, 20000)
...
for (num_samples in sample_sizes) {
  ...
  for (trial in 1:num_trials) {
    ...
    dual_pval.orig <- weighted_lr_test(dt, weights=dual_weights)
    
    primal_weights <- parametric_primal_weights(dt)
    primal_pval.orig <- weighted_lr_test(dt, weights=primal_weights)
...
save(list = c("orig.result"),file = "results.RData")
