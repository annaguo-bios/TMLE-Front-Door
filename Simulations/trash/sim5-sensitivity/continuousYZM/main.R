...
test <- function(dt, i , m.level=NULL , estimator='AIPW'){
  ...
  ## p(m|A,z)
  m.fit <- glm(formula = M ~ A + Z, data = dt, family = gaussian())
  
  dt.z1 <- dt %>% mutate(Z=1)
  dt.z0 <- dt %>% mutate(Z=0)
  
  E.m.Az1 <- predict(m.fit, newdata = dt.z1, type = "response")
  E.m.Az0 <- predict(m.fit, newdata = dt.z0, type = "response")
  E.m.AZ <- predict(m.fit, type = "response")
  sd.m.Az1 <- sqrt(var(M - E.m.Az1))
  sd.m.Az0 <- sqrt(var(M - E.m.Az0))
  sd.m.AZ <- sqrt(var(M - E.m.AZ))
  
  p.m1.Az1 <- pnorm(0, E.m.Az1, sd.m.Az1, lower.tail = T)
  p.m1.Az0 <- pnorm(0, E.m.Az0, sd.m.Az0, lower.tail = T)
  p.m1.AZ <- pnorm(0, E.m.AZ, sd.m.AZ, lower.tail = T)
