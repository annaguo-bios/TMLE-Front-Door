fd_admg1 <- function(num_samples, lb=1, ub=2) {
  ...
  M <- rnorm(num_samples, -0.5 - betaZM * Z + betaAM * A, 1)
...
fd_admg2 <- function(num_samples, lb=1, ub=2) {
  ...
  Z <- rnorm(num_samples, 0.5 + alphaU3Z * U3 - alphaU4Z * U4, 1)
  ...
  betaAM <- runif(1, lb, ub)
  M <- rnorm(num_samples, -0.5 - alphaU3M * U3 + alphaU4M * U4 + betaAM * A, 1)
...
nonfd_admg1 <- function(num_samples, lb=1, ub=2) {
  ...
  M <- rnorm(num_samples, -0.5 - betaZM * Z + betaAM * A + alphaU1M * U1 - alphaU2M * U2, 1)
