...
echo "Enter the name for this simulation:"
read sim_input
...
for i in $(seq 1 ${n}); do
    python "$dsq_path/dSQ.py" --job-file ${prefix}_n$i.txt --batch-file ${prefix}_n$i.sh --job-name "${sim_input}-$i" --mail-type ALL -o dsq/dsq-jobfile-%A_%a-%N.out
done
...
sed -i '1a #SBATCH --partition=day-long-cpu,preemptable' ${prefix}_n$i.sh
done
