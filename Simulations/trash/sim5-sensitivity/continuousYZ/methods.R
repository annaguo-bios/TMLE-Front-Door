parametric_dual_weights <- function(dt, M, mpM, A, a=1, M.type='binary'){
  
  M.family <- if(M.type == 'binary'){binomial()}else{gaussian()} # binomal() if M is binary, gaussian() if M is continuous
  
  modelM <- glm(formula = as.formula("M~."), data = dt[,c(M,mpM), drop=F], family = M.family)
  
  # p(M|A,X)
  p.M1.AX <- predict(modelM, type = "response")
  p.M.AX <- dt[[M]]*p.M1.AX + (1-dt[[M]])*(1-p.M1.AX)
  
  #p(M|a,X)
  dt.a <- dt %>% mutate(!!A := a)
  p.M1.aX <- predict(modelM, newdata = dt.a, type = "response")
  p.M.aX <- dt[[M]]*p.M1.aX + (1-dt[[M]])*(1-p.M1.aX)
