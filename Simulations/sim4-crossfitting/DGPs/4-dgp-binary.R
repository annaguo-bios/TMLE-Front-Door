generate_data <- function(n,parA = c(0.48, 0.07, 1.00, -1.00, -0.34, -0.12, 0.30, -0.35, 1.00, -0.10, 0.46, # linear terms
  ...
  #               -0.4, -0.4, 0.0, 0.4, 0.0, 0.2, 1.0, -0.3, 1.0, -0.4), nrow = 10, byrow = TRUE)
  
  
  # X <- mvrnorm(n , mu =c(0.42, -0.12, 0.05, 0.20, 0.32, -0.12, -0.08, -0.04, -0.04, 0.48) , Sigma = (t(V)%*%(V))) # p(X)
  ...
  M <- rbinom(n,1,plogis(parM[1] + parM[2]*A + rowSums(sweep(X, 2, parM[3:12], "*")) + rowSums(diag(A)%*%sweep(X[,1:5],2,parM[13:17],"*")) + rowSums(sweep(X[,6:10]^2,2,parM[18:22],"*")) )) # p(M|A,X)
  ...
  m.ratio.a1 <- dbinom(M,1,plogis(parM[1] + parM[2]*1 + rowSums(sweep(X, 2, parM[3:12], "*")) + rowSums(diag(rep(1,n))%*%sweep(X[,1:5],2,parM[13:17],"*")) + rowSums(sweep(X[,6:10]^2,2,parM[18:22],"*")) ))/
  ...
  m.ratio.a0 <- dbinom(M,1,plogis(parM[1] + parM[2]*0 + rowSums(sweep(X, 2, parM[3:12], "*")) + rowSums(diag(rep(0,n))%*%sweep(X[,1:5],2,parM[13:17],"*")) + rowSums(sweep(X[,6:10]^2,2,parM[18:22],"*")) ))/
    dbinom(M,1,plogis(parM[1] + parM[2]*A + rowSums(sweep(X, 2, parM[3:12], "*")) + rowSums(diag(A)%*%sweep(X[,1:5],2,parM[13:17],"*")) + rowSums(sweep(X[,6:10]^2,2,parM[18:22],"*")) ))
