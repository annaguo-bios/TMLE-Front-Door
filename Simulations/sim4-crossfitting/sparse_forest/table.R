...
setwd(here('./sim4-crossfitting/sparse_forest'))
...
# Binary M, Onestep, SL -----------------
load("binaryM/RF/Onestep-est1/result.Rdata")
...
load("binaryM/RF/TMLE-est1/result.Rdata")
...
load("continuousM/RF/Onestep-est2a/result.Rdata")
for (obj in objs) {
...
load("continuousM/RF/Onestep-est2b/result.Rdata")
...
load("continuousM/RF/TMLE-est2a/result.Rdata")
...
load("continuousM/RF/TMLE-est2b/result.Rdata")
...
table1 <- as_hux(dat) %>% 
  ...
  set_caption("Impact of cross-fitting on TMLE and one-step ATE estimators using random forests (RF: 200 trees; min node size = 1; CF: 5-fold cross-fitting).") %>% set_col_width(1/14*c(2,rep(1,12)))
