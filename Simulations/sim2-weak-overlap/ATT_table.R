...
#   set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>% set_caption("Comparison between TMLEs and one-step estimators for ATT under positivity assumption violation with binary, continuous, and multivariate mediators.") %>%
...
table1 <- as_hux(dat) %>% 
  ...
  # insert_row("","","","","","","","","","","","",after=8) %>%
  # insert_row("","","","","","","","","","","","",after=14) %>%
  insert_column(c("","","Bias","SD","MSE","CI coverage","CI width","Bias","SD","MSE","CI coverage","CI width","Bias","SD","MSE","CI coverage","CI width"), after = 0) %>%
  ...
  set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>%
  set_top_border(row=1,col=everywhere,brdr(1, "solid")) %>% set_bottom_border(row = nrow(.),col = everywhere,brdr(1, "solid")) %>%
  set_escape_contents(nrow(.), 1:ncol(.), FALSE) %>% set_caption("Comparison of TMLE and one-step ATT estimators under weak overlap across mediator types.") %>%
  set_all_padding(1) %>% set_font_size(6)
