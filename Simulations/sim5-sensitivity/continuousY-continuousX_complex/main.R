...
source("DGPs/DGPs.R")
source("methods.R")
r_pkg_location <- paste0(dirname(dirname(here())),'/drinf')
devtools::load_all(r_pkg_location)
...
f.CATE <- function(dt, i , m.level=NULL){ # estimator: np, AIPW, IPW, Gcomp
  ...
  ## p(Z|X)
  model_Z <- glm(Z ~ X, data = dt, family = binomial(link = "logit"))
  
  p.z1.X <- predict(model_Z, type = "response")
  p.z0.X <- 1- p.z1.X
  
  ## p(A|Z,X)
  model_A <- glm(A ~ Z*X, data = dt, family = binomial(link = "logit"))
...
dr.test.z1m1 <- drinf.tmle(
  ...
  tolQ = 1e-3,
  maxIter = 5,
  tolIF = 1/(sqrt(length(dt[,1]))), 
  verbose=TRUE,
  return.naive = TRUE,
  cvFolds = 1
)

dr.test.z0m1 <- drinf.tmle(
  L0 = dt[,"X", drop = FALSE], 
  L1 = dt$A, 
  L2 = dt$Y, 
  A0 = dt$Z, 
...
dr.test.m1 <- list(est.diff=dr.test.z1m1$est.ltmle-dr.test.z0m1$est.ltmle, std.diff=sqrt(mean((dr.test.z1m1$ic_full - dr.test.z0m1$ic_full)^2)/nrow(dt)))

## DR test
dr.test.z1m0 <- drinf.tmle(
  L0 = dt[,"X", drop = FALSE], 
  L1 = dt$A, 
  L2 = dt$Y, 
  A0 = dt$Z, 
...
dr.test.m0 <- list(est.diff=dr.test.z1m0$est.ltmle-dr.test.z0m0$est.ltmle, std.diff=sqrt(mean((dr.test.z1m0$ic_full - dr.test.z0m0$ic_full)^2)/nrow(dt)))

dr.wald.test <- any(
  c(
    (dr.test.m1$est.diff - qnorm(1-0.05/4)*dr.test.m1$std.diff > 0) | (dr.test.m1$est.diff + qnorm(1-0.05/4)*dr.test.m1$std.diff < 0),
    (dr.test.m0$est.diff - qnorm(1-0.05/4)*dr.test.m0$std.diff > 0) | (dr.test.m0$est.diff + qnorm(1-0.05/4)*dr.test.m0$std.diff < 0)
  )
)
...
save(list = c("dr.wald.test","wald.test","p.test","d.test"),file = paste0(dgp.f.name,"/output/output_n",n,"_",estimator,"_seed",seed,".Rdata"))