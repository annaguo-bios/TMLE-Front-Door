...
dr.cate.wald.matrix <- matrix(nrow = nsim, ncol = length(n.vec))

dr.cate.wald.rate_matrix <- data.frame(n=n.vec, # sample size
                               type1error_power=vector(mode="integer",length=length(n.vec))) # true positive rate
...
for (i in seq_along(n.vec)){
  ...
  for (seed in 1:nsim){
    ...
    dr.cate.wald.matrix[seed,i] <- dr.wald.test
    ## weight based
  ...
  # record rate
  dr.cate.wald.rate_matrix[i,"type1error_power"] <- mean(dr.cate.wald.matrix[,i])
...
# save data
save(list = c("dr.cate.wald.matrix","dr.cate.wald.rate_matrix",
              "cate.wald.matrix","cate.wald.rate_matrix",
