fd_admg1 <- function(num_samples, sd.X=1,sd.Y=1) {
  ...
  Ybeta_intercept <- -0.5
  Ybeta_M <- -0.5
  Ybeta_U1 <- 0 # changed here from 1 to 0
  Ybeta_X <- 0.5
  Ybeta_MU1 <- 1.2
  Ybeta_MX <- -1.5
  Ybeta_U1X <- 1.5
...
fd_admg2 <- function(num_samples, sd.X=1,sd.Y=1) {
  ...
  Z <- rbinom(num_samples, 1, (Zbeta_intercept + Zbeta_X * X + Zbeta_U2 * U2 )/4)
  ...
  A <- rbinom(num_samples, 1, (Abeta_intercept + Abeta_Z * Z + Abeta_X * X + Abeta_U1 * U1)/4)
  ...
  M <- rbinom(num_samples, 1, plogis(Mbeta_intercept + Mbeta_A * A + Mbeta_X * X + Mbeta_U2 * U2 ))
...
# Due to A<->M, M<->Y
nonfd_admg1 <- function(num_samples, sd.X=0.5,sd.Y=0.5) { 
  ...
  M <- rbinom(num_samples, 1, plogis(Mbeta_intercept + Mbeta_A * A + Mbeta_Z * Z + Mbeta_X * X + Mbeta_U1 * U1 + Mbeta_U2*U2 ))
