...
parametric_primal_weights <- function(dt,Y.family){
  ...
  pY.a1 <- dt$Y*EY.a1 + (1-dt$Y)*(1-EY.a1)
  pY.a0 <- dt$Y*EY.a0 + (1-dt$Y)*(1-EY.a0)
  pY <- dt$Y*EY + (1-dt$Y)*(1-EY)
...
# p.test <- t.test(abs(p.pred1 - dt$Y), abs(p.pred0 - dt$Y), paired = TRUE)
# d.test <- t.test(abs(d.pred1 - dt$Y), abs(d.pred0 - dt$Y), paired = TRUE)
